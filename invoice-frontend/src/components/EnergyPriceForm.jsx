// src/components/EnergyPriceForm.jsx
...
const EnergyPriceForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Precios de energía guardados con éxito'); // Mensaje de éxito
      ...
      if (error.response) {
        ...
        toast.error(`Error: ${error.response.data.message}`); // Mensaje de error
      } else {
        console.error('Error al guardar los precios de energía:', error.message);
        toast.error('Error de conexión con el servidor'); // Mensaje de error
      }
    }
  };
  ...
  return (
    <div className="mt-4">
      <form onSubmit={handleSubmit} className="border p-4 rounded bg-light shadow">
        <h2 className="mb-3 text-center" style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>Introducir Precios de Energía</h2>
